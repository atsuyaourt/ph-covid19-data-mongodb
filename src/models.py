...
HEALTH_STATS_ENUM = ["recovered", "died", "confirmed", "asymptomatic", "mild", "moderate", "severe", "critical"]
        ...
        if col_name in CASE_SCHEMA:
            if CASE_SCHEMA[col_name]["dtype"] == "String":
                if "choices" in CASE_SCHEMA[col_name]:
                    ...
                    no_match = df.dropna().loc[~df[col_name].isin(CASE_SCHEMA[col_name]["choices"]), col_name].unique()
                    if len(no_match) > 0:
                        print(f"{col_name} has unmatched values: {no_match}!!! exiting...")
                        valid = False
                        break
                if "default" in CASE_SCHEMA[col_name]:
                    df.loc[df[col_name].isna(), col_name] = CASE_SCHEMA[col_name]["default"]
                else:
